...
export const Home: React.FC<HomeProps> = ({ onOpenBooking }) => {
  ...
  useEffect(() => {
    ...
    const subscription = client.listen(query).subscribe((update) => {
      if (update.result) {
        ...
        // If the document is deleted, update.result might be null, so we refetch just in case
  ...
  const activeCoreValues = cmsData?.coreValues || defaultCoreValues;
  ...
  const activeProcessSteps = cmsData?.processSteps?.map((step: any) => ({ step: step.stepNumber, title: step.title, desc: step.description })) || defaultProcessSteps;
    ...
    <div className="w-full">
          ...
          <div className="lg:col-span-6 flex flex-col items-start text-left space-y-6">
            ...
            >
              ...
              <span className="font-serif italic text-xs md:text-sm text-luxury-gold tracking-[0.25em] uppercase">
                {cmsData?.heroTagline || "Some Feelings Deserve More Than Words"}
              </span>
            </motion.div>
            ...
            >
              {cmsData?.heroTitle ? (
                // Split title if it contains spaces to recreate the stylized effect
                cmsData.heroTitle.split(' ').map((word: string, i: number, arr: any[]) => (
                  <React.Fragment key={i}>
                    {i === arr.length - 1 ? <span className="text-luxury-gold">{word}</span> : word}
                    {i === 0 && <br />}
                    {i !== 0 && i !== arr.length - 1 && ' '}
                  </React.Fragment>
                ))
            ...
            >
              ...
              >
                {cmsData?.heroButton1Text || "Plan Your Celebration"}
              </button>
              <button
              ...
              >
                {cmsData?.heroButton2Text || "Explore Our Services"}
              </button>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-6 relative flex justify-center items-center mt-12 lg:mt-0 select-none">
            ...
            >
              <div
                ...
                style={{ backgroundImage: !cmsData ? 'none' : `url(${cmsData?.heroImage ? urlFor(cmsData.heroImage).url() : proposalHero})` }}
              />
            </motion.div>
            ...
            >
              <div
                ...
                style={{ backgroundImage: !cmsData ? 'none' : `url(${cmsData?.heroImageSmall ? urlFor(cmsData.heroImageSmall).url() : weddingGold})` }}
              />
            </motion.div>
            ...
            >
              {cmsData?.heroImageTag || "Bespoke Surprises"}
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col space-y-6">
            <SectionHeader
              title="Our Story"
              tagline={cmsData?.storyTagline || "Every celebration begins with a feeling."}
              align="left"
            />
            {cmsData?.storyDescription ? (
              // Very simple portable text rendering for just text blocks
              cmsData.storyDescription.map((block: any, i: number) => (
            ...
            <blockquote className="border-l-2 border-luxury-gold pl-6 my-6 font-serif italic text-base md:text-lg tracking-wider text-luxury-gold leading-[1.6]">
              "{cmsData?.storyQuote || "The most precious moments in life are the ones shared with the people we love."}"
            </blockquote>
      ...
      <section className="bg-luxury-warm/50 border-y border-luxury-gold/10 py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-10">
          {activeCoreValues.map((value: any, idx: number) => (
            <motion.div
              key={value.title}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="w-12 h-12 rounded-full border border-luxury-gold/30 flex items-center justify-center bg-white">
                {value.icon || defaultCoreValues[idx % defaultCoreValues.length].icon}
              </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12">
        <SectionHeader
          title={cmsData?.servicesTitle || "Bespoke Experiences"}
          tagline="What we plan for you"
          subtitle={cmsData?.servicesDescription || "Explore our curated event styles. We specialize in tailoring custom layouts for every life landmark."}
        />
      ...
      <section className="bg-luxury-dark text-white py-24 border-y border-luxury-gold/15">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <SectionHeader
            title={cmsData?.processTitle || "How We Work"}
            tagline={cmsData?.processTagline || "Seamless Planning, Step by Step"}
            subtitle={cmsData?.processSubtitle || "Our structured luxury process guarantees that your vision is realized flawlessly, keeping you stress-free."}
            dark
          />
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 relative flex justify-center order-2 lg:order-1">
            <div className="w-[300px] h-[400px] border-gold-double relative p-3">
              <div
                ...
                style={{ backgroundImage: `url(${cmsData?.whyImage ? urlFor(cmsData.whyImage).url() : corporateGala})` }}
              />
            </div>
            {/* Floating Card */}
            <div className="absolute top-10 -right-10 bg-white/90 backdrop-blur-md border border-luxury-gold/30 p-6 rounded shadow-lg hidden sm:block max-w-[200px]">
              <span className="font-serif text-3xl font-bold text-luxury-gold">{cmsData?.whyStatNumber || "100%"}</span>
              <p className="font-sans text-[10px] uppercase tracking-widest text-luxury-charcoal font-medium mt-1">
                {cmsData?.whyStatText || "Stress-Free Coordination Guarantee"}
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col space-y-6 order-1 lg:order-2">
            <SectionHeader
              title={cmsData?.whyTitle || "Why Aysh Evermore?"}
              tagline={cmsData?.whyTagline || "Because special moments deserve more than ordinary planning."}
              align="left"
            />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs text-luxury-charcoal/70 leading-relaxed font-light tracking-wide">
              {cmsData?.whyReasons ? cmsData.whyReasons.map((reason: any, idx: number) => {
                 const DefaultIcons = [Compass, Sparkles, ShieldCheck, Heart];
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12">
        <SectionHeader
          title={cmsData?.journalTitle || "Atelier Journal"}
          tagline={cmsData?.journalTagline || "Instagram Visual Story"}
          subtitle={cmsData?.journalSubtitle || "Follow our visual board on Instagram @aysh.evermore to discover our latest setups, aesthetics, and moments."}
        />

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-8">
          {(cmsData?.journalImages && cmsData.journalImages.length > 0
            ? cmsData.journalImages
            : [proposalHero, weddingGold, birthdayLuxury, surpriseGarden]
          ).map((img: any, idx: number) => {
            const bgUrl = typeof img === 'string' ? img : urlFor(img).url();
            return (
              <a
                key={idx}
      ...
      <section className="bg-luxury-warm/20 py-24 border-t border-luxury-gold/10">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-5 flex flex-col space-y-8">
            <SectionHeader
              title={cmsData?.contactTitle || "Let's Plan"}
              tagline={cmsData?.contactTagline || "Let's create your moment."}
              align="left"
            />

            <p className="font-sans text-xs md:text-sm text-luxury-charcoal/70 leading-relaxed font-light tracking-wide max-w-md">
              {cmsData?.contactDescription || "Every celebration starts with a feeling, a dream, a person, a moment worth remembering. Reach out to us, and let's begin crafting yours."}
            </p>
