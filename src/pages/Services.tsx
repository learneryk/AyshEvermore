    ...
    <div className="w-full pt-28 pb-20 select-text">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 flex flex-col space-y-6">
          <div className="border border-luxury-gold/20 p-2 rounded bg-white shadow-md flex items-center justify-center overflow-hidden">
            <img 
              src={activeCategory.image && typeof activeCategory.image !== 'string' ? urlFor(activeCategory.image).url() : (activeCategory.image || defaultServicesData.find(s => s.slug === activeCategory.slug)?.image)}
              alt={activeCategory.name}
              className="w-full h-auto rounded-sm transition-all duration-700 object-contain max-h-[400px]"
            />
          </div>
